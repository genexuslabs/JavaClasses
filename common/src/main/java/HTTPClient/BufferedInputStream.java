...
class BufferedInputStream extends FilterInputStream
{
	...
	// optimize for large reads//@gusbro  : Esto es para que no vaya a leer m�s de 512Kb (en Win95 puede fallar)
	if (pos >= end  &&  len >= lr_thrshld  &&  mark_pos < 0)
        return in.read(buf, off, (len < 524288 ? len : 524288));
//@gusbro\
...
//@gusbro    int got = in.read(buffer, pos, (buffer.length - pos < 524288 ? buffer.length - pos : 524288));
//@gusbro\	if (got > 0)
	    end = pos + got;
    }

...
//@gusbro\
    ...
    public HTTPResponse Delete(String file, byte data[], NVPair[] headers)
		throws IOException, ModuleException
		{
    	return setupRequest("DELETE", stripRef(file), headers, data, null);
		}
...
//@gusbro -- Comento todos estos metodos y agrego algunos

    /** Setea si se desea usar pipelining o no
     */
    public static void setPipelining(boolean doPipeline)
    {
        if(doPipeline == neverPipeline)
            Log.write(Log.CONN, "Conn:  "  + (doPipeline ? "enabling" : "disabling") + "  pipelining");
        neverPipeline = !doPipeline;
    }

    /** Indica si se est� usando pipelining
     */
    ...
    public void setSSLConnection(ISSLConnection sslConnection)
    {
    ...
    /**
     * Set the ssl context for the current connection.
     *
     * @param the SSLContest to use
     */
/*    public void setSSLContext(SSLContext context)
    {
	this.ssl_ctxt = context;
    }

    /**
     * Get the current ssl context.
     *
     * @return the current SSLContest
     */
...
//@gusbro\
    ...
    public void addDigestProxyAuthorization(String realm, String user, String passwd)
    {
    	String ProxyHost;
    	if (Proxy_Host != null)
	...
	AuthorizationInfo.addDigestAuthorization(ProxyHost, ProxyPort, realm, user,
						 passwd, getContext());
    }
    ...
    public void addBasicProxyAuthorization(String realm, String user, String passwd)
    {
    	String ProxyHost;
    	if (Proxy_Host != null)
    		ProxyHost = Proxy_Host;
	...
	AuthorizationInfo.addBasicAuthorization(ProxyHost, ProxyPort, realm, user,
						passwd, getContext());
    }
	...
	try
	{
		...
		{
			resp = new HTTPResponse(gen_mod_insts(), Timeout, req);
		...
		else
        {
            try
            {
				resp = new HTTPResponse(gen_mod_insts(), Timeout, req);
				handleRequest(req, resp, null, true);
                int statusCode = resp.getStatusCode(); // Puede que aqui ocurra una excepci�n (generalmente java.io.EOFException)
                if(statusCode >= 500)throw new Exception("SanGuchito"); // O que el statusCode sea mayor que 500 (problemas con el proxy)
            }catch(Exception e)
            {
                String proxies = System.getProperties().getProperty("http.nonProxyHosts", "").trim();
                String host = getHost();
                String tempProxies = ("|" + proxies + "|").toUpperCase();

                // Primero chequeamos que este host no est� ya dentro del Don't Proxy
                if(tempProxies.indexOf("|" + host.toUpperCase() + "|") == -1)
                { // OK, no estaba en la lista
					Log.write(Log.CONN, "Conn: Attempting to bypass proxy server [" + Proxy_Host + ":" + Proxy_Port + "]. Target host:" + getHost() + ":" + getPort());
                    // Entonces intentamos primero *solo* con este request a ver si era un problema de proxys
                    String realProxy_Host = this.Proxy_Host;
                    int realProxy_Port = this.Proxy_Port;
                    setCurrentProxy(null, 0);
                    HTTPResponse probe = new HTTPResponse(gen_mod_insts(), Timeout, req);
                    handleRequest(req, probe, null, true);
                    try
                    {
                        if(probe.getStatusCode() >= 500) throw new Exception("Condorito"); // Si falla tambien aca es porque no era problema del proxy
                        resp = probe;
                        // Si pasamos esa l�nea, es porque el problema estaba con el Proxy, as� que metemos este Host a la lista de Don't proxies
                        Log.write(Log.CONN, "Conn: Automatically adding server to ProxyBypassList: " + host);
                        if(!proxies.endsWith("|"))proxies += "|";
                        proxies += host;
                        System.getProperties().put("http.nonProxyHosts", proxies);
                        try
						{
							HTTPConnection.dontProxyFor(host);
						}catch(Exception cannotDontProxy)
						{
							Log.write(Log.CONN, "Conn: Could't add dontProxyFor " + host);
						}
                    }catch(Exception noEraProblemaDelProxy)
                    {
						Log.write(Log.CONN, "Conn: Rolling back proxy bypassing. Proxy: " + realProxy_Host + ":" + realProxy_Port);
                        // Si no era problema del Proxy, hacemos 'rollback' de esto, y dejamos todo como estaba
                    }finally
					{ // Sea como fuere, seteamos para atras los datos del proxy
					  // Si pudimos salir sin usar el proxy esto queda en las dontProxyFor(host)
					  // asi la proxima vez no trata de usar el proxy
                        setCurrentProxy(realProxy_Host, realProxy_Port);
					}
                }
				else
				{
					Log.write(Log.CONN, "Conn: Already attempted to bypass proxy server [" + Proxy_Host + ":" + Proxy_Port + ". Target host:" + getHost() + ":" + getPort());
				}
				if(resp == null)
				{ // Podr�a ocurrir que al reintentar sin proxy todav�a siga sin tener una HTTPResponse
				  // En el caso en que todav�a no tenga HTTPResponse, la pido de nuevo
				  // Si aqui ocurre una excepci�n es porque no se puede llegar al host ni pasando ni sin pasar por el proxy
					Log.write(Log.CONN, "Conn: Retrying setupRequest");
					resp = new HTTPResponse(gen_mod_insts(), Timeout, req);
					handleRequest(req, resp, null, true);
			    }
...
//@gusbro
                        sock = sslConnection.processSSLSocket(sock, Host, Port);

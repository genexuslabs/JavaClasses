...
public class Spelling {
    ...
    public boolean buildDictionary() {
        try {
            IndexReader my_luceneReader = IndexReader.open(Settings.getInstance().
                    getIndexFolder());
            SpellChecker spell = getSpelling();
            if (spell != null)
                spell.indexDictionary(new LuceneDictionary(my_luceneReader,
                        IndexRecord.CONTENTFIELD));
    ...
    private SpellChecker getSpelling() {
        try {
            String dictionaryFolder = Settings.getInstance().getIndexFolder() +

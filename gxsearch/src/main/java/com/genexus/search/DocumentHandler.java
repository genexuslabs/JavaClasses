...
public class DocumentHandler {
    ...
    public static String htmlCleanFile(String fileName) {
        try {
            return new JTidyHTMLHandler().htmlClean(new FileInputStream(
                    fileName));
    ...
    public static String htmlPreview(Object obj, String query, String textType, String preTag, String postTag, int fragmentSize, int maxNumFragments)
    {
            ...
            try {
                ...
                if (!query.equals("") && !text.equals("")){
                    ...
                    TokenStream tokenStream = Indexer.CreateAnalyzer().
                                              tokenStream(
                            IndexRecord.CONTENTFIELD, new StringReader(text));

                    String result = highlighter.getBestFragments(tokenStream,
                            text, maxNumFragments,
                            "...");
                    return result;
                }else
                {

...
public class ExcelCells implements IExcelCells {
	...
	public ExcelCells(IGxError errAccess, ExcelDocument document, Workbook workBook,
			org.apache.poi.ss.usermodel.Sheet selectedSheet, int rowPos, int colPos, int height, int width,
	...
	public ExcelCells(IGxError errAccess, ExcelDocument document, Workbook workBook,
			org.apache.poi.ss.usermodel.Sheet selectedSheet, int rowPos, int colPos, int height, int width,
			boolean readonly, StylesCache stylesCache) {
		doc = document;
	...
	protected Font getInternalFont(boolean b, short color, short fontHeight, String name, boolean italic,
	...
	public short getUnderline() {
		if (pWorkbook.getFontAt(pCells[1].getCellStyle().getFontIndexAsInt()).getUnderline() != XSSFFont.U_NONE) {
			return 1;
		}
		return 0;
	}
			...
			for (int i = 1; i <= cntCells; i++) {
															...
															// color Index de Excel y voy por el else
				{
					if (fontColor == null
						...
						if ((red + green + blue) != 0) {
							newColor = new XSSFColor(new java.awt.Color(red, green, blue),null);

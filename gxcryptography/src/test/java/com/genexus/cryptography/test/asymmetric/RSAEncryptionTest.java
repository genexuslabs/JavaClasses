...
public class RSAEncryptionTest extends SecurityAPITestObject {
	...
	private void bulkTest(PrivateKeyManager privateKey, PublicKey cert, String hashAlgorithm, boolean isPublicKey) {
		...
		for (String arrayPadding : arrayPaddings) {
			...
			String encrypted1 = asymCipher.doEncrypt_WithPrivateKey(hashAlgorithm, arrayPadding, privateKey,
				plainText);
			String decrypted1 = isPublicKey ? asymCipher.doDecrypt_WithPublicKey(hashAlgorithm, arrayPadding, cert, encrypted1) : asymCipher.doDecrypt_WithCertificate(hashAlgorithm, arrayPadding, (CertificateX509) cert, encrypted1);
			Assert.assertEquals(decrypted1, plainText);
			True(SecurityUtils.compareStrings(decrypted1, plainText), asymCipher);
			String encrypted2 = isPublicKey ? asymCipher.doEncrypt_WithPublicKey(hashAlgorithm, arrayPadding, cert, plainText) : asymCipher.doEncrypt_WithCertificate(hashAlgorithm, arrayPadding, (CertificateX509) cert, plainText);
			String decrypted2 = asymCipher.doDecrypt_WithPrivateKey(hashAlgorithm, arrayPadding, privateKey,
				encrypted2);
